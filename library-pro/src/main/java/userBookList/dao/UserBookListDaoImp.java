...
public class UserBookListDaoImp implements UserBookListDAO{
	...
	// 현재 대출/예약중인 도서 조회
	@Override
	public List<UserBookListDTO> CurrBorrowList(String user_id) {
		return sqlSession.selectList("userBookList.currBorrowList", user_id);
	}
	
	// 대출 내역 조회
	@Override
	public List<UserBookListDTO> PastBorrowList(String user_id) {
		return sqlSession.selectList("userBookList.pastBorrowList", user_id);
	}
	...
	public UserBookListDTO Return(int borrow_keynum) {
		...
		//예약자 정보 가져오기
		return sqlSession.selectOne("userBookList.selectReserveExist", sqlSession.selectOne("userBookList.selectBookKeynum", borrow_keynum));
	}
