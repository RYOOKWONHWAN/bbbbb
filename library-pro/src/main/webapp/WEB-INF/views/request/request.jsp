				...
				<div class="request_form">
					...
					<!--  action="${pageContext.request.contextPath}/request/request/requestInsert" -->
						...
						<form name="selectfrm" id="selectfrm" method="post">
							...
							<button type="submit" id="requestInsert">도서신청</button>
						</form>
					</div>
				</div>
	        ...
	        $.ajax({
	            ...
	            success:function(data){
					...
					$.each(bookData.documents, function(index, element) {
						$('.request_bookset .inner').append(`<div class='request_book'><input name='all_data` + test +`' type='hidden' value='`+ JSON.stringify(element) +`' /><img src=' `+ element.thumbnail +` ' alt='책 표지' /><div class='request_text'><h4>` + element.title + `</h4><p>저자 : `+ element.authors +`</p><p>출판사 : `+ element.publisher +`</p><p>출판년도 : `+ element.datetime +`</p><p> ISBN: `+ element.isbn +`</p><button class='data_btn' name='btnSelect' type='button'>선택</button></div></div>`)
