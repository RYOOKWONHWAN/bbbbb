	...
	<div class="inner">
		...
		<div class="contents my_area">
			...
			<div class="my_tab_cont_3 active recommend_area">
				<h4>
					${user_name}님과 유사한 <span>${user_age} ${user_sex}</span>의<br />
					인기도서입니다.
				</h4>
				<table>
					<tr>
						...
						<tr>
							...
							<td>
								<form method="GET" action="/myapp/books/search">
									...
									<input type="hidden" name="categories" value="a" /> <input
										type="hidden" name="query" value="${dto.isbn }" />

									<button type="submit">통합 도서 조회하기</button>
								</form>
							</td>
							<td><a href="${pageContext.request.contextPath}/request/request">도서
									신청하기</a></td>
						</tr>
					</c:forEach>
				</table>
				...
				<c:choose>
					<c:when test="${borrow_count == 0}">
						<h4>아직 반납하신 도서가 없습니다. 책을 읽고 도서를 추천받아보세요</h4>
					</c:when>

					<c:when test="${borrow_count != 0}">
						<h4>
							최근 읽은 "<span>${lastBookTitle}</span>"(와)과<br /> 유사한 도서입니다.
						</h4>
						...
						<div class="recommend_book_area">
							<c:forEach items="${recommendList}" var="dto">
								<div class="recommend_book">
									...
									<ul>
										<li><a
											href="${pageContext.request.contextPath}/books/view?page=${dto.book_keynum} ">${dto.title}</a></li>
										<li>${dto.author}</li>
									</ul>
								</div>
							</c:forEach>
