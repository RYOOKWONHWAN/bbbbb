...
<section class="subpage_wrap">
	<div class="inner">
		...
		<div class="contents book_cont_view">
			...
			<p class="book_sub">
				저자 : ${bldto.author } <span></span> 출판사 : ${bldto.publisher }
			</p>
			<div class="book_info_area">
				<img src="${bldto.thumbnail }" alt="책 표지" />
				<p class="book_info_text">
					ISBN : <span>${bldto.isbn }</span> <br /> 분류기호 : <span>${bldto.category_s }</span>
					<br /> KDC 분류 : <span>${bldto.book_category }</span> <br /> 출판일자
					: <span>${bldto.pub_date }</span> <br /> 등록일자 : <span><fmt:formatDate
							value="${bldto.receive_date }" pattern="yyyy-MM-dd" /></span>
				</p>
				<!-- 후기 평균 별점 구하기 -->
				...
				<c:if test="${numReviews > 0}">
					...
					<fmt:formatNumber var="avgStarsInt" value="${avgStars}"
						type="number" maxFractionDigits="0" />
				</c:if>
				<c:if test="${numReviews eq 0}">
					<c:set var="avgStars" value="${numReviews }"></c:set>
					<fmt:formatNumber var="avgStarsInt" value="${numReviews }"
						type="number" maxFractionDigits="0" />
				</c:if>
				<div class="book_state_area">
					<p class="book_star star_${avgStarsInt }">
						<span>${avgStarsInt} (${pv.totalCount })</span> <img
							src="/myapp/resources/assets/images/star-full.svg"
							alt="star_full" /> <img
							src="/myapp/resources/assets/images/star-full.svg"
							alt="star_full" /> <img
							src="/myapp/resources/assets/images/star-full.svg"
							...
							src="/myapp/resources/assets/images/star-full.svg"
							alt="star_full" /> <img
							src="/myapp/resources/assets/images/star-none.svg"
							alt="star_none" /> <img
							src="/myapp/resources/assets/images/star-none.svg"
							alt="star_none" /> <img
							src="/myapp/resources/assets/images/star-none.svg"
			...
			<div class="review_area">
				<form name="frm" id="reviewForm" action="writeRev" method="post">
					<ul>
						...
						<li>
							<!-- cols="50" rows="3" --> <textarea id="reviewText"
								class="review_input" name="review_contents"
				...
				<h4 class="review_title">
					도서 리뷰 <span>(${pv.totalCount }건)</span>
				</h4>
				...
				<c:forEach items="${revList}" var="dto">
					...
					<div class="review_box">
						<p class="book_star star_${dto.star_num}">
							<img src="/myapp/resources/assets/images/star-full.svg"
								alt="star_full" /> <img
								src="/myapp/resources/assets/images/star-full.svg"
								alt="star_full" /> <img
								src="/myapp/resources/assets/images/star-full.svg"
						...
						<p class="review_text">
							${dto.review_contents} <br /> <span>작성자:
						...
						<div class="review_box hide">
							<form id="review_${dto.review_keynum}" class="updateReview"
								...
								<ul>
									<li>
										<h4>
											리뷰 수정 <span id="close_${dto.review_keynum}">[수정 취소]</span>
										</h4>
									</li>
									<li><select name="star_num">
											<option value="5">⭐⭐⭐⭐⭐</option>
									...
									<li><textarea class="review_input"
			...
			<div class="pagination_area">
				<ul class="pagination">
					...
					<c:choose>
						<c:when test="${pv.startPage >1}">
							<li class="page-item disabled"><a class="page-link"
								style="cursor: pointer;" href="view?currentPage=1"> <img
									style="opacity: .5;"
					...
					<c:choose>
						<c:when test="${pv.startPage >1}">
							<li class="page-item disabled"><a class="page-link"
								...
								href="view?currentPage=${pv.startPage-pv.blockPage}"> <img
									style="opacity: .5;"
						...
						<c:choose>
							<c:when test="${i==pv.currentPage}">
								<li class="page-item active"><a class="page-link"
									style="cursor: pointer;" href="view?currentPage=${i}">${i}</a>
								</li>
							</c:when>
							<c:otherwise>
					...
					<c:choose>
						<c:when test="${pv.endPage < pv.totalPage}">
							<li class="page-item"><a class="page-link"
								style="cursor: pointer;" href="view?currentPage=${pv.totalPage}">
									<img style="opacity: .5;"
