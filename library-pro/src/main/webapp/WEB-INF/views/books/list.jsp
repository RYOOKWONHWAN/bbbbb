	...
	<div class="inner">
		...
		<div class="contents books_cont">
			...
			<div class="books_tab_cont_1 active">
				...
				<p class="bn_area">
					<a class="bn_a" onclick="sendData('a')">전체</a> <span>·</span> <a
						class="bn_0" onclick="sendData('0')">총류</a> <span>·</span> <a
						class="bn_1" onclick="sendData('1')">철학</a> <span>·</span> <a
						class="bn_2" onclick="sendData('2')">종교</a> <span>·</span> <br />
					<a class="bn_3" onclick="sendData('3')">사회과학</a> <span>·</span> <a
						class="bn_4" onclick="sendData('4')">순수과학</a> <span>·</span> <a
						class="bn_5" onclick="sendData('5')">기술과학</a> <span>·</span> <br />
					<a class="bn_6" onclick="sendData('6')">예술</a> <span>·</span> <a
						class="bn_7" onclick="sendData('7')">언어</a> <span>·</span> <a
						class="bn_8" onclick="sendData('8')">문학</a> <span>·</span> <a
						class="bn_9" onclick="sendData('9')">역사</a>
				</p>
				<c:if test="${empty printdtos && empty search_result}">
					<h4 class="booklist_text">
						<!-- 총 <span>0000</span>건 -->
						검색 결과가 없습니다. <span class="search_text"></span>
					</h4>
				</c:if>
					...
					<c:choose>
						...
						<c:when test="${not empty  search_result}">
							<c:if test="${not empty query  }">
								<h4 class="booklist_text">
									<!-- 총 <span>0000</span>건 -->
									"<span class="search_text">${query }</span>"에 대해 <span>${count}</span>건의
									결과가 있습니다.
								</h4>
							</c:if>
							...
							<c:forEach var="dto" items="${search_result }">
								...
								<div class="list">
									<a
										href="${pageContext.request.contextPath}/books/view?page=${dto.book_keynum}"><img
								...
								<c:when test="${not empty searchResultPv }">
									<c:choose>
										<c:when test="${searchResultPv.startPage >1}">
											<li class="page-item disabled"><a class="page-link"
												...
												href="search?search_item=${option}&query=${query}&categories=${cate}&searchbutton=&currentPage=1">
													<img style="opacity: .5;"
									...
									<c:choose>
										<c:when test="${searchResultPv.startPage >1}">
											<li class="page-item disabled"><a class="page-link"
												style="cursor: pointer;"
												href="search?search_item=${option}&query=${query}&categories=${cate}&searchbutton=&currentPage=${searchResultPv.startPage-searchResultPv.blockPage}">
													<img style="opacity: .5;"
										...
										<c:choose>
											<c:when test="${i==searchResultPv.currentPage}">
												<li class="page-item active"><a class="page-link"
													style="cursor: pointer;"
													href="search?search_item=${option}&query=${query}&categories=${cate}&searchbutton=&currentPage=${i}">${i}</a>
												</li>
											</c:when>
											<c:otherwise>
									...
									<c:choose>
										<c:when
											...
											<li class="page-item"><a class="page-link"
												style="cursor: pointer;"
												href="search?search_item=${option}&query=${query}&categories=${cate}&searchbutton=&currentPage=${searchResultPv.totalPage}">
													<img style="opacity: .5;"
								...
								<c:when test="${not empty pv}">
									<c:choose>
										<c:when test="${pv.startPage >1}">
											<li class="page-item disabled"><a class="page-link"
												style="cursor: pointer;" href="?currentPage=1"> <img
													style="opacity: .5;"
													src="/myapp/resources/assets/images/first.svg"> <span>first</span>
											</a></li>
										</c:when>
									...
									<c:choose>
										<c:when test="${pv.startPage >1}">
											<li class="page-item disabled"><a class="page-link"
												style="cursor: pointer;"
												href="?currentPage=${pv.startPage-pv.blockPage}"> <img
													style="opacity: .5;"
													src="/myapp/resources/assets/images/prev.svg"> <span>prev</span>
											</a></li>
										</c:when>
									...
									<c:forEach var="i" begin="${pv.startPage}" end="${pv.endPage}">
										<c:choose>
											<c:when test="${i==pv.currentPage}">
												<li class="page-item active"><a class="page-link"
													style="cursor: pointer;" href="?currentPage=${i}">${i}</a>
												</li>
											</c:when>
											<c:otherwise>
												<li class="page-item"><a class="page-link"
									...
									<c:choose>
										<c:when test="${pv.endPage < pv.totalPage}">
											<li class="page-item"><a class="page-link"
												style="cursor: pointer;" href="?currentPage=${pv.totalPage}">
													<img style="opacity: .5;"
													src="/myapp/resources/assets/images/last.svg"> <span>last</span>
											</a></li>
										</c:when>
